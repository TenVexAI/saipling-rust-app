...
import { deleteBook } from '../../utils/tauri';
...
export function DeleteBookModal({ bookId, bookTitle, onClose, onDeleted }: DeleteBookModalProps) {
  const projectDir = useProjectStore((s) => s.projectDir);
  const [confirmation, setConfirmation] = useState('');
  const [deleting, setDeleting] = useState(false);
  const [error, setError] = useState('');

  const expectedPhrase = `delete ${bookTitle}`;
  ...
  const handleDelete = async () => {
    ...
    setError('');
    try {
      await deleteBook(projectDir, bookId);
      onDeleted();
      onClose();
    } catch (e) {
      setError(String(e));
      ...
      >
        ...
        <div className="rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)', padding: '14px', marginBottom: '16px' }}>
          <p className="text-sm" style={{ color: 'var(--text-primary)', marginBottom: '8px' }}>
            This will permanently delete <strong>{bookTitle}</strong> and all of its chapters, scenes, and files.
          </p>
        ...
        <div style={{ marginBottom: '16px' }}>
          <label className="block text-xs font-medium" style={{ color: 'var(--text-secondary)', marginBottom: '6px' }}>
            Type <strong style={{ color: 'var(--text-primary)' }}>delete {bookTitle}</strong> to confirm
          </label>
          <input
            value={confirmation}
            onChange={(e) => setConfirmation(e.target.value)}
            placeholder={`delete ${bookTitle}`}
